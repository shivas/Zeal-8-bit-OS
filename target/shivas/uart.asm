...
; SPDX-License-Identifier: Apache-2.0
        ...
        UART_PORT: EQU SIO_A_DATA             ; UART0
...
;
UART0_serial_RX:
        in a, (SIO_A_CTRL)       ; Get the line status register
        and     %00000001        ; Check for characters in buffer
        ret z                    ; Just ret (with carry clear) if no characters
        in a, (SIO_A_DATA)        ; Read the character from the UART receive buffer
...
UART0_serial_TX:        PUSH            BC                      ; Stack BC
                        PUSH            AF                      ; Stack AF
; Check whether we're clear to send (UART only)
;
...
UART0_serial_TX1:
                        IN             A,(SIO_A_CTRL)       ; Get the line status register
                        and            %00000100            ; Check for TX empty
...
UART0_serial_TX2:       POP            AF                      ; Good to send at this point, so
                        out     (SIO_A_DATA), a       ; Write the character to the UART transmit buffer
                        POP            BC                      ; Restore BC
                        SCF                                     ; Set the carry flag
                        RET 
